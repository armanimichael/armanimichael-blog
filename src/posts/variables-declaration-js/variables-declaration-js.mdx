...
[JavaScript](/category/javascript/) has been around since **1995**. That's certainly a long time for a programming language to be around.
...
Function variable can also work this way, which is also what we should avoid as it's kind of confusing:
...
The `const` keyword defines a constant, which can't be re-declared and, it read-only. Note that constants are not immutable. You can still edit objects and arrays.
